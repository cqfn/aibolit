...
`Train` command does the following:
 ...
 5. You have to specify train and test dataset: set the `HOME_TRAIN_DATASET` environment variable 
 ...
 6. If you need to set up own directory where model will be saved, set up also `SAVE_MODEL_FOLDER` environment variable.
 Otherwise model will be saved into `cloned_aibolit_path/aibolit/binary_files/model.pkl`
 7. If you need to set up own folder with Java files, use `--java_folder parameter`, the default value will be `scripts/target/01` of aibolit cloned repo
 8. You need to install Java 13 and Maven
