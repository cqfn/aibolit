 ...
 */
public abstract class BaseKeyframeAnimationManager<K, A> {
  ...
  /**
   * Returns the progress into the current keyframe between 0 and 1. This does not
   * take into account any interpolation that the keyframe may have.
   */
  float getLinearCurrentKeyframeProgress() {
    ...
    final float progressIntoFrame = progress - keyframe.getStartProgress();
    final float keyframeProgress = keyframe.getEndProgress() - keyframe.getStartProgress();
    return progressIntoFrame / keyframeProgress;
  }

  /**
   * Takes the value of {@link #getLinearCurrentKeyframeProgress()} and
   * interpolates it with the current keyframe's interpolator.
   */
  ...
  public A getValue() {
    final float progress = getInterpolatedCurrentKeyframeProgress();
    if (valueCallback == null && keyframesWrapper.isCachedValueEnabled(progress)) {
      return cachedGetValue;
    }

    final Keyframe<K> keyframe = getCurrentKeyframe();
    final A value = getValue(keyframe, progress);
  ...
  public void setValueCallback(@Nullable final LottieValueCallback<A> valueCallback) {
  ...
  /**
   * keyframeProgress will be [0, 1] unless the interpolator has overshoot in
   * which case, this should be able to handle values outside of that range.
   */
  abstract A getValue(Keyframe<K> keyframe, float keyframeProgress);

  private static <T> KeyframesWrapper<T> wrap(final List<? extends Keyframe<T>> keyframes) {
  ...
  private static final class EmptyKeyframeWrapperListener<T> implements KeyframesWrapper<T> {
    @Override
    ...
    @Override
    public boolean isValueChanged(final float progress) {
      return false;
    }
    ...
    @Override
    public boolean isCachedValueEnabled(final float interpolatedProgress) {
      throw new IllegalStateException("not implemented");
    }
  }
  ...
  private static final class SingleKeyframeWrapper<T> implements KeyframesWrapper<T> {
    ...
    SingleKeyframeWrapper(final List<? extends Keyframe<T>> keyframes) {
  ...
  private static final class KeyframesWrapperImpl<T> implements KeyframesWrapper<T> {
    ...
    KeyframesWrapperImpl(final List<? extends Keyframe<T>> keyframes) {
    ...
    private Keyframe<T> findKeyframe(final float progress) {
